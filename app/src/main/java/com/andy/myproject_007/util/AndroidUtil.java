...
public class AndroidUtil {
     ...
     *
     * @param resId
     * @param status  true:删除图标 false:其他图标
     * @param tv      图标textview
     * @param context
     */
    ...
    public static Bitmap compressImage(String path) {
        ...
        //开始读入图片，此时把options.inJustDecodeBounds 设回true了,如果我们把它设为true，
        // 那么BitmapFactory.decodeFile(path, options)并不会真的返回一个Bitmap给你，
    ...
    public static Bitmap compressImage(Bitmap image) {
        ...
        while (baos.toByteArray().length / 1024 > 100) {    //循环判断如果压缩后图片是否大于100kb,大于继续压缩
            ...
            image.compress(CompressFormat.JPEG, inSampleSize, baos);//inSampleSize%，把压缩后的数据存放到baos中
            inSampleSize -= 10;//每次都减少10
        }
